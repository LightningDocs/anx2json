...
def find_answer(name_tag: str, answer_set: ET = tree.getroot()):
    ...
    if _type == "TextValue":
        ...
        if not mc_as_list:
            if len(child) == 1:
                ...
                raise Exception(
                    f'expected only 1 selection, but found {len(child)} in the answer "{name_tag}"'
                )
        ...
        raise Exception(
            f"um this is weird this should never pop up. The type was {_type}"
        )
...
def parse_interestStepSpreadsheet(duration_cmp_name, rate_cmp_name):
    ...
    if (not parse_primitive("Interest Step TF")) or (
        (interest_step_durations_element is None)
        and (interest_step_rates_element is None)
    ):
        return None
    ...
    if interest_step_durations_element is not None:
        durations_as_list = [
            parse_NumValue(duration.text if duration is not None else None)
            for duration in interest_step_durations_element[0]
        ]
    else:
        durations_as_list = []
    ...
    if interest_step_rates_element is not None:
        rates_as_list = [
            parse_NumValue(rate.text) for rate in interest_step_rates_element[0]
        ]
    else:
        rates_as_list = []
...
def parse_Completion(percent_cmp_name: str, days_cmp_name: str) -> list:
    ...
    if contract_percent_element is not None:
        percents_as_list = [
            parse_NumValue(percent.text) for percent in contract_percent_element[0]
        ]
    else:
        percents_as_list = []
...
def parse_completionGuarantors(guarantor_cmp_name: str) -> list:
    ...
    if construction_guaranty_name_element is None:
        return None
...
def parse_lenderUCCInformation(
    lender_name_cmp_name: str, is_individual_cmp_name: str
) -> list:
    ...
    if is_individual_element is not None:
        is_individuals_as_list = [
            parse_TFValue(individual.text) for individual in is_individual_element[0]
        ]
    else:
        is_individuals_as_list = []

    result = []
    for name, is_individual in zip_longest(
        names_as_list, is_individuals_as_list, fillvalue=None
    ):
        temp_dict = {}
        if name is not None:
            temp_dict["name"] = name
...
def parse_DebtServiceType(
    interest_reserve_cmp_name: str, interest_reserve_months_cmp_name: str
) -> str:
    ...
    if (interest_reserve_element is None) or (
        interest_reserve_element[0].text == "false"
    ):
        return "None"
...
def parse_security_OwnershipSpreadsheet(
    name_cmp: str,
    individual_cmp: str,
    signer_1_cmp: str,
    title_cmp: str,
    ...
    if individual_element is not None:
        individuals_list = [
            parse_TFValue(individual.text) for individual in individual_element[0]
        ]
    else:
        individuals_list = []

    if signer_1_element is not None:
        signers_list = [parse_TextValue(signer.text) for signer in signer_1_element[0]]
    else:
        signers_list = []

    if title_element is not None:
        titles_list = [parse_TextValue(title.text) for title in title_element[0]]
    else:
        titles_list = []

    if state_element is not None:
        states_list = [
            parse_TextValue(state[0].text) for state in state_element[0] if len(state)
        ]
    else:
        states_list = []

    result = []
    for name, individual, signer, title, state in zip_longest(
        names_list, individuals_list, signers_list, titles_list, states_list
    ):
        temp_dict = {}
        if name is not None:
            temp_dict["name"] = name
        if individual is not None:
            temp_dict["isIndividual"] = individual
        if signer is not None:
            temp_dict["signerForPledgor"] = signer
...
def parse_Security_CSADebtorSpreadsheet(
    name_cmp: str, individual_cmp: str, title_cmp: str, signer_cmp: str, state_cmp: str
) -> list[dict]:
    ...
    if signer_element is not None:
        signers_list = [parse_TextValue(signer.text) for signer in signer_element[0]]
    else:
        signers_list = []
    ...
    result = []
    for name, individual, title, signer, state in zip_longest(
        names_list, individuals_list, titles_list, signers_list, states_list
    ):
        temp_dict = {}
        if name is not None:
            temp_dict["debtorNameWithVesting"] = name
        if individual is not None:
            temp_dict["isIndividual"] = individual
        if title is not None:
            temp_dict["title"] = title
        if signer is not None:
            temp_dict["signerForDebtor"] = signer
        if state is not None:
            temp_dict["state"] = state
...
def parse_MultipleLenders(name_cmp: str, amount_cmp: str) -> list[dict]:
    ...
    if (name_element is None) and (amount_element is None):
        return None
...
def parse_isEscrow(dropdown_cmp: str) -> bool:
    ...
    if selection == "Escrow and Title":
        ...
        raise Exception(
            'i have no idea how I got here. The selection was "{selection}"'
        )


def parse_FeesSpreadsheet(
    fee_cmp: str, description_cmp: str, comment_cmp: str, paid_to_cmp: str = None
) -> list[dict]:
    ...
    if description_element is not None:
        descriptions_list = [
            parse_TextValue(description.text) for description in description_element[0]
        ]
    else:
        descriptions_list = []
    ...
    if comment_element is not None:
        ...
        comments_list = [
            comment[0].text if len(comment) else None for comment in comment_element[0]
        ]
    else:
        comments_list = []
    ...
    result = []
    for fee, description, comment, paid_to in zip_longest(
        fees_list, descriptions_list, comments_list, paid_to_list
    ):
        temp_dict = {}
        if fee is not None:
            temp_dict["amount"] = fee
        if description is not None:
            temp_dict["description"] = description
        if comment is not None:
            temp_dict["comment"] = comment
        if paid_to is not None:
            temp_dict["paidTo"] = paid_to
...
def parse_geraciFee(fee_cmp: str) -> int:
    ...
    if fee_element is None:
        return None

    if len(fee_element[0]) != 1:
        raise Exception(
            f"thats weird, there should really only be one value for {fee_cmp}, but there was {len(fee_element[0])}"
        )
...
def parse_geraciFeeDelivery(delivery_cmp: str) -> str:
    ...
    if len(delivery_element[0]) != 1:
        raise Exception(
            f"thats weird, there should really only be one value for {delivery_cmp}, but there was actually {len(delivery_element[0])}"
        )

    return parse_TextValue(delivery_element[0][0][0].text)


def parse_assignment_Spreadsheet_list(
    property_cmp: str,
    manager_cmp: str,
    date_cmp: str,
    street_cmp: str,
    ...
    if property_element is not None:
        properties_list = [
            prop[0].text if len(prop) else None for prop in property_element[0]
        ]

    managers_list = []
    if manager_element is not None:
        managers_list = [
            parse_TextValue(manager.text) for manager in manager_element[0]
        ]

    dates_list = []
    if date_element is not None:
        dates_list = [parse_DateValue(date.text) for date in date_element[0]]

    streets_list = []
    if street_element is not None:
        streets_list = [parse_TextValue(street.text) for street in street_element[0]]

    cities_list = []
    if city_element is not None:
        cities_list = [parse_TextValue(city.text) for city in city_element[0]]
    ...
    if zip_element is not None:
        zips_list = [parse_TextValue(zipcode.text) for zipcode in zip_element[0]]

    result = []
    for property, manager, date, street, city, state, zipcode in zip_longest(
        properties_list,
        managers_list,
        dates_list,
        streets_list,
        ...
        if property is not None:
            temp_dict["Property"] = property
        if manager is not None:
            temp_dict["propertyManager"] = manager
        if date is not None:
            temp_dict["agreementDate"] = date
        address = {}
...
def parse_subordinations_list(
    docs_cmp: str,
    property_cmp: str,
    post_closing_cmp: str,
    document_name_cmp: str,
    document_date_cmp: str,
    months_cmp: str,
    tenants_cmp: str,
) -> list[dict]:
    ...
    if post_closing_element is not None:
        post_closing_as_list = [
            parse_TFValue(answer.text) for answer in post_closing_element[0]
        ]
    ...
    if document_date_element is not None:
        document_date_as_list = [
            parse_DateValue(date.text) for date in document_date_element[0]
        ]

    months_as_list = []
    if months_element is not None:
        months_as_list = [
            parse_NumValue(num_months.text) for num_months in months_element[0]
        ]

    tenants_as_list = []
    if tenants_element is not None:
        for RpTValue in tenants_element[0][
            :-1
        ]:  # Use '[:-1]' to exclude the last element, since it is always unanswered / unnecessary
            if len(RpTValue) == 0:
                tenants_as_list.append(None)
            else:
                tenants = [
                    tenant.text for tenant in RpTValue if tenant.text is not None
                ]
                if len(tenants):
                    tenants_as_list.append(tenants)
                else:
                    tenants_as_list.append(None)
    ...
    for (
        ...
        temp_dict = {}
        if docs is not None:
            temp_dict["documentType"] = docs
        if property is not None:
            temp_dict["property"] = property
        if post_closing is not None:
            temp_dict["postClosing"] = post_closing
        if document_name is not None:
            temp_dict["documentName"] = document_name
        if document_date is not None:
            temp_dict["documentDate"] = document_date
        if months is not None:
            temp_dict["leaseMonths"] = months
        if tenants is not None:
            temp_dict["tenantNames"] = tenants
...
def parse_isIntercreditors(loan_docs_cmp: str) -> bool:
    loan_docs_elem = find_answer(loan_docs_cmp)

    if loan_docs_elem is None:
        return None

    selected_choices = [selection.text for selection in loan_docs_elem[0]]
    if "Intercreditor" in selected_choices:
        return True
    else:
        return False


def parse_intercreditors_list(
    rep_options_cmp: str,
    doc_type_cmp: str,
    doc_recording_cmp: str,
    debt_amt_cmp: str,
    recording_date_cmp: str,
    signing_date_cmp: str,
    instrument_num_cmp: str,
    trustor_cmp: str,
    trustee_cmp: str,
    street_cmp: str,
    city_cmp: str,
    ...
    rep_options_element = find_answer(rep_options_cmp)
    doc_type_element = find_answer(doc_type_cmp)
    doc_recording_element = find_answer(doc_recording_cmp)
    debt_amt_element = find_answer(debt_amt_cmp)
    recording_date_element = find_answer(recording_date_cmp)
    signing_date_element = find_answer(signing_date_cmp)
    instrument_num_element = find_answer(instrument_num_cmp)
    trustor_element = find_answer(trustor_cmp)
    trustee_element = find_answer(trustee_cmp)
    street_element = find_answer(street_cmp)
    city_element = find_answer(city_cmp)
    state_element = find_answer(state_cmp)
    zip_element = find_answer(zip_cmp)
    ...
    rep_options_as_list = []
    if rep_options_element is not None:
        for MCValue in rep_options_element[0]:
            if len(MCValue) == 0:
                rep_options_as_list.append(None)
            else:
                rep_options_as_list.append(MCValue[0].text)
